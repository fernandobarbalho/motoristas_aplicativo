...
calcula_arrecadacao_total <- function(media_horas_trabalhadas_semanais, parametros) {
  ...
  resultados$cenario <- paste(media_horas_trabalhadas_semanais,"horas")
...
cenarios<-  purrr::map_dfr(30:52, function(horas_semanais){
    print(horas_semanais)
    resultado = calcula_arrecadacao_total(horas_semanais, parametros)
    as_tibble(resultado)
...
cenarios_trabalho<-
cenarios%>%
  pivot_longer(cols = 2:11, names_to = "medida", values_to = "valor") %>%
  summarise(lim_inferior = min(valor),
          lim_superior =max(valor),
          media = mean(valor),
          .by = c(cenario, medida))
...
dados<-
cenarios_trabalho %>%
  filter(medida == "total_arrecadacao_mensal") %>%
  mutate(media= media/10^6,
         color = ifelse(cenario == "40 horas", "orange", ifelse(cenario == "52 horas", "red", "white"))) 

colors = ifelse(dados$cenario == "40 horas", "orange", 
               ifelse(dados$cenario == "52 horas", "red", "white"))


dados%>%
  ggplot(aes(y=cenario, x=media)) +
  geom_col(fill = colors) +
  geom_text(aes(label = paste(round(media, 1))), hjust = 1.2, size = 3, color = "black") +
  theme_light() +
  theme(
...
##Comparações de receita arrecadada anual para diversos cenários

```{r}
dados<-
cenarios_trabalho %>%
  filter(medida == "total_arrecadacao_anual") %>%
...
dados%>%
  ...
  labs(
    title = "Estimativa de arrecadação anual total",
...
dados<-
cenarios_trabalho %>%
  filter(medida == "horas_trabalhadas_semanais") 
...
dados%>%
  ggplot(aes(y=cenario, x=media), ) +
  geom_col(fill = colors) +
  #geom_text(aes(label = paste(round(media, 1))), hjust = 3, size = 3, color = "black") +
  geom_errorbar(aes(xmin = lim_inferior, xmax = lim_superior),color= "blue", size=1) +
  theme_light() +
  theme(
    panel.background = element_rect(fill="black"),
    panel.grid = element_blank()
...
dados<-
cenarios_trabalho %>%
  filter(medida == "horas_trabalhadas_mensais") 
...
dados%>%
  ggplot(aes(y=cenario, x=media), ) +
  geom_col(fill = colors) +
  geom_text(aes(label = paste(round(media, 1))), hjust = 2, size = 3, color = "black") +
  geom_errorbar(aes(xmin = lim_inferior, xmax = lim_superior),color= "blue", size=1) +
  theme_light() +
  theme(
    panel.background = element_rect(fill="black"),
...
dados<-
cenarios_trabalho %>%
  filter(medida == "remuneracao_minima") 
...
dados%>%
  ggplot(aes(y=cenario, x=media), ) +
  geom_col(fill = colors) +
  geom_text(aes(label = format(round(media,2), decimal.mark = ",", big.mark = ".")), hjust = 1.8, size = 3, color = "black") +
  geom_errorbar(aes(xmin = lim_inferior, xmax = lim_superior),color= "blue", size=1) +
  theme_light() +
  theme(
    panel.background = element_rect(fill="black"),
...
dados<-
cenarios_trabalho %>%
  filter(medida == "salario_contribuicao") 
...
dados<-
cenarios_trabalho %>%
  filter(medida == "contribuicao_empresa") 
...
dados%>%
  ...
  labs(
    title = "Estimativa de contribuição de empresas por autônomo",
    x= "",
    y="cenário"
  )
...
dados<-
cenarios_trabalho %>%
  filter(medida == "contribuicao_autonomo") 
...
dados%>%
  ...
  labs(
    title = "Estimativa de contribuição de autônomo",
    x= "",
    y="cenário"
  )
...
dados<-
cenarios_trabalho %>%
  filter(medida == "remuneracao_liquida_previdencia") 
